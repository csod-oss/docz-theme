...
export interface RenderState {
  ...
  width: string;
  height: string;
  showing: "jsx" | "html" | "variations" | "props";
  code: string;
  key: number;
}
...
export class Render extends Component<RenderComponentProps, RenderState> {
  ...
  get actions(): JSX.Element {
    ...
    return (
      <Actions>
        <Tabs>
          {variations && (
            <Tab active={showing === "variations"} onClick={showVariations}>
              VARIATIONS
            </Tab>
          )}
          {propsTable && (
            <Tab active={showing === "props"} onClick={showProps}>
              PROPS
            </Tab>
          )}
  ...
  public render(): JSX.Element {
    const { className, style, scope, variations, propsTable } = this.props;
                ...
                <Container>
                  <ul style={{ listStyleType: "none", margin: 0, padding: 0 }}>
                    {variations &&
                      variations.map(
                        (
                          variation: { name: string; component: string },
                          index: number
                        ) => {
                          return (
                            <li key={index}>
  ...
  private setVariation(component: string): void {
    this.setState({ code: component });
  }
